...
def write_candidate_report(data_dir: Path, con: sqlite3.Connection, limit: int = 25) -> None:
...
def main() -> None:
    ...
    # candidate queue: score >= 40 aur unique tag ke saath (triage me)
    cur.execute("DELETE FROM candidate_findings")
    seen = set()
    for x in scored:
        if x["score"] >= 40 and (x["url"], x["tag"]) not in seen:
            seen.add((x["url"], x["tag"]))
            cur.execute(
                "INSERT OR REPLACE INTO candidate_findings "
    ...
    write_candidate_report(data_dir, con)
    con.close()
